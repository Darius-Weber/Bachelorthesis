...
#from models.utils import scatter_sum
...
class GCNConv(MessagePassing):
    ...
    def forward(self, x, edge_index, edge_attr):
        x = (self.lin_src(x[0]), x[1]) #x[0] source node features, x[1] destination node features

        if edge_attr is not None and hasattr(self, 'lin_edge'):
            edge_attr = self.lin_edge(edge_attr)

        row, col = edge_index #source and destination nodes
