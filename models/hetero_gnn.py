...
class TripartiteHeteroGNN(torch.nn.Module):
    def __init__(self,
                 ...
                 num_mlp_layers,
                 dropout,
                 #share_conv_weight, In code of paper always false
                 #share_lin_weight,  In code of paper always false
                 use_norm,
                 use_res,
                 #in_place=True,
        ...
        #if pe_dim > 0: Laplacian eigenvector
        #    self.pe_encoder = torch.nn.ModuleDict({ #heterogenous operation
        ...
        #consists of three Multi-Layer Perceptrons (MLP), each of which is designed to encode one type of node in the graph:
        # 'vals', 'cons', and 'obj'
        ...
        self.encoder = torch.nn.ModuleDict({'vals': MLP([in_shape, hid_dim, in_emb_dim], norm='batch'),
                                            ...
                                            'obj': MLP([in_shape, hid_dim, in_emb_dim], norm='batch')}) #default is already batch
        ...
        self.gcns = torch.nn.ModuleList() #empty modullist
    ...
    def forward(self, data):
        ...
        for i in range(self.num_layers):
            ...
            #h1 = x_dict
            h2 = self.gcns[i](x_dict, edge_index_dict, edge_attr_dict) #dictionary with HeteroConv to different edge types (hetero_conv is applied!)
            keys = h2.keys()
            hiddens.append((h2['vals'])) #h2['cons'])) #h2['obj']))
            #if self.use_res: #in paper code it is always false.
        ...
        #obj = torch.cat([self.pred_obj[i](obj[i]) for i in range(self.num_layers)], dim=1)
        return vals
